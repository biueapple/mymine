...
//숨을 곳을 찾으며 전진 (방법을 생각해봤는데 일단은 팀원중에 방패가 있다면 그걸 사용하고 없다면 그냥 전진 하는걸로)
//가까워 지면 공격
//체력이 적어지면 도망
//도망을 가다가 숨어야 하나?
public class WarriorGoblin : Enemy, ICoveredEnemy, IFlockingEnemy
{
    ...
    new void Start()
    {
        ...
        BehaviorTreeSequence shieldColleagueSequence = new(new List<BehaviorTreeNode> { shieldColleagueNode, shieldColleagueMoveNode });

        //보유중인 추격방법 2가지중 가능한것을 실행
        BehaviorTreeSelector chaseasg = new(new List<BehaviorTreeNode> { chaseNode, shieldColleagueSequence });
        ...
        BehaviorTreeSequence chaseSequence = new(new List<BehaviorTreeNode> { chaseInRange, chaseasg });
